...
async def handle_request_9091(request):
    message = await request.text()
    print("CloudAnalyzer received message from SDV2:")
    print(message)
    ...
    async with ClientSession() as session:
        try:
            ...
            async with session.post(f'http://{SDVUser_IP}:{SDVUser_port}/receive_message', data=message) as response:
                ...
                print(f"Forwarded to SDVUser({SDVUser_IP}): {response.status}, {await response.text()}")
