...
def receive_message():
    ...
    # 컴퓨터 5로 speed 메시지 전송
    url = "http://10.60.0.4:9090/receive_message"
    response = requests.post(url, data=message, headers={'Content-Type': 'text/plain'}, timeout=1)

    return Response("Message received", status=200)

if __name__ == '__main__':
    app.run(debug=True, host='10.60.0.2', port=8081)
